...
function userExists(userName, callback) {
    return userIdExists(MD5(userName.toLowerCase()), callback);
}
    ...
    userExists(userName, function(err, result) {
        ...
        bcrypt.hash(password, 12, function(err, hash) {
            ...
            users.run("\
                ...
                {
                    $userid: MD5(userName.toLowerCase()),
                    $username: userName,
                    $email: email,
                    $password: hash
                }, function(err){
...
function deleteUser(userName, callback){
    deleteUserId(MD5(userName.toLowerCase()), callback);
}
...
function deleteUserId(userId, callback){
    userIdExists(userId, function(err, result) {
        ...
        users = new sqlite3.Database('./db/users.db');
        users.run("DELETE FROM users WHERE userID = ?", userId, function(err){
            users.close();
            if (err) return callback(err);
            return callback(null, true);
        });
...
function authUser(userName, password, callback){
    authUserId(MD5(userName.toLowerCase()), password, callback);
}
...
function authUserId(userID, password, callback){
    users = new sqlite3.Database('./db/users.db');
    users.all("SELECT * FROM users WHERE userID = ?", userID, function (err, rows) {
        users.close();
        if (err) return callback(err);
        if (rows[0]){
